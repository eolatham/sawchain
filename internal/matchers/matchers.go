...
// StatusConditionMatcher is a Gomega matcher that checks if a Kubernetes resource has a specific status condition
...
func (m *StatusConditionMatcher) FailureMessage(actual interface{}) string {
	return fmt.Sprintf("Expected\n\t%#v\nto have status condition %s=%s", actual, m.ConditionType, m.ExpectedStatus)
}

// NegatedFailureMessage implements the Gomega matcher interface
func (m *StatusConditionMatcher) NegatedFailureMessage(actual interface{}) string {
	return fmt.Sprintf("Expected\n\t%#v\nnot to have status condition %s=%s", actual, m.ConditionType, m.ExpectedStatus)
}
